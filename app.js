...
function allowedErrors(len) { return len<=4 ? 0 : len<=7 ? 1 : len<=11 ? 2 : 3; }
function fuzzyMatch(input, target) {
  const iTok = input.split(/[\s,\/]+/).filter(Boolean);
  const tTok = target.split(/[,\/]+/).map(s=>s.trim()).filter(Boolean);
  return iTok.some(a => tTok.some(c => levenshtein(a, c) <= allowedErrors(Math.max(a.length, c.length))));
}
